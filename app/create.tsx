...
import { Subtask, Task, TaskPriority } from "../src/types/task";

export default function CreateTaskScreen() {
  const router = useRouter();
  const { addCategory, addTask, categories, isTasksLoaded } = useTasks();
  ...
  const handleBreakdown = async () => {
    ...
    try {
      ...
      setSubtasks(suggestions.map((s, i) => ({
        id: `${Date.now()}-${i}`,
        title: s,
        completed: false,
      })));
    } catch (e: any) {
      setAiError(e?.message ?? "Failed to get suggestions. Please try again.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 20, backgroundColor: "#f8f8fb" }}>
            ...
            {subtasks.map((subtask, index) => (
              ...
              >
                ...
                <Pressable onPress={() => removeSubtask(subtask.id)}>
                  <Text style={{ color: "#eb5757", fontWeight: "bold", fontSize: 16 }}>✕</Text>
                </Pressable>
              </View>
            ))}
          </View>
      ...
      {pendingTask ? (
        <View style={{ backgroundColor: "#fef2f2", borderRadius: 8, padding: 12, marginBottom: 16, borderWidth: 1, borderColor: "#fca5a5" }}>
          <Text style={{ color: "#dc2626", fontWeight: "bold", fontSize: 14 }}>⚠ Duplicate Task</Text>
          <Text style={{ color: "#dc2626", fontSize: 13, marginTop: 4, marginBottom: 10 }}>
            A task named "{pendingTask.title}" already exists. Do you still want to create it?
          </Text>
          <View style={{ flexDirection: "row", gap: 8 }}>
            <Pressable onPress={() => setPendingTask(null)} style={{ flex: 1, backgroundColor: "#e8e8ef", padding: 10, borderRadius: 8, alignItems: "center" }}>
              <Text style={{ fontWeight: "bold", color: "#333" }}>Cancel</Text>
            </Pressable>
            <Pressable onPress={handleCreateAnyway} style={{ flex: 1, backgroundColor: "#dc2626", padding: 10, borderRadius: 8, alignItems: "center" }}>
